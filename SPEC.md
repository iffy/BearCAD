...
- **Operation identity (#1055):** `Document::boolean_ops` is an `arena::Arena` too, keyed by
  `BooleanOpKey` — what `BodySource::Boolean`, `SceneElement::BooleanOp` and
  `ComponentMember::BooleanOp` hold. Deleting a Combine removes it. The rule that an
  operation may not consume its own result used to be spelled as an index comparison
  ("a later op"); it is now the descendant walk it always meant — the input must not be one
  of the op's outputs, nor anything built from them.
