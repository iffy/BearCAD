...
- **DEV menu (#627), debug builds only** (`cargo run`; not user-facing, not on the
  docs-site): a native **DEV** menu with **Report issue…**, opening a window **inside the main
  one** with a focused description textarea and
  ...
  *Include the document JSON*. **Submit** (or **Cmd/Ctrl+Enter** from inside the
  ...
  both written to temp files. The form **steps aside** for the two frames of the capture
  (`hide_frames`) so the picture shows the app rather than the form describing it, and comes
  back whether or not the picture arrives; a running script only takes the frame when it asked
  for one itself (`ScriptRunner::wants_screenshot`). It's a window in the main one rather than
  its own OS window (#872) because a second native window puts a second font atlas on the one
  texture egui shares between viewports, and the next glyph typed anywhere lands outside it.
  The window stays open after a submit — cleared, re-focused,
...
- **Import/Export menus & toolbar (#352):** the File menu groups the model interchange under an
  **Import** submenu (BearCAD/STL/STEP/Image) and an **Export** submenu (STL/STEP), and the model
