...
- **Graph rollback (#524/#531):** a rollback marker lets you view the model as it was just
  after a chosen element, temporarily suppressing everything that **depends on** it. Selecting a
  single element shows a **⏮ Roll back to here** button atop the Elements pane; the marker then
  reads as `⏮ Rolled back to <name>` with a **Clear** button to roll forward. Suppression is by
  ...
  marker's **descendants** — so two independent branches never affect each other, and rolling
  back a body hides exactly the operations built on it and their results. Those elements are
