...
- **Context pane:** shows the **union** of editable properties for everything currently
  ...
  so inputs line up down the whole pane. **Checkbox toggles use the same two columns** (#588,
  `context::checkbox_row`): the label (plus any shortcut hint) in the left column, the box in the
  right, and **clicking either the label or the box** toggles it. Applies to the draw-tool toggles
  (Construction, Snapping, Curve, Tangent), Extrude/Revolve **Symmetric**, and Slice **Infinite cut**.
- **Primary button (#586):** a tool that completes an action shows a single **blue, no-text**
  commit button (a checkmark icon) in the **right column** of the two-column layout
  ...
  tool's readiness (e.g. Revolve needs a profile + axis; Loft needs ≥ 2 sections).
...
- **Graph rollback (#524/#531/#545):** a rollback marker lets you view the model as it was just
  ...
  back, a `⏮ Rolled back to [just before] <name>` status with a **Clear** button (to roll
...
- Coverage includes **every modeling tool** — the sketch tools plus Extrude, Chamfer, Fillet,
  Offset, Project, Loft, Revolve, Sweep, Combine, Move, Mirror, Repeat, Slice, and Text (each
