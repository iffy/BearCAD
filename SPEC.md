...
- **Export session commands:** **Help → Export Session Commands…** (also a command-palette
  ...
  **Recorded as Lua, not as instructions (#1070).** An instruction naming an element has to be
  spelled while a document is at hand to count live elements against, so `CommandLog` renders
  each one at record time (`Instruction::as_lua_in`) and keeps the text. `as_lua()` alone can
  only say the arena **slot**, which matches the ordinal a replay resolves right up until
  something of that kind is deleted — delete image 0 and the survivor is ordinal 0 while its
  slot is still 1, so an export naming the slot replays to nothing. The camera flush still
  renders with no document, which is safe because a camera instruction names no elements.
...
- **Elements pane view modes (#34/#252):** two icon-toggle buttons next to the pane heading
  ...
  **taller** rather than overflowing sideways (`declutter_label_bands` returns each node's x and
  ...
  drawing, exports, Move-to-component, Rollback, Delete), plus the drawing rename menu and
  edge-treatment Edit on their display-only nodes. This is a per-session UI preference, not saved with the document.
