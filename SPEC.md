...
- **Every tool input is an element picker (#534):** anything the user can click to feed a
  ...
  exception. Model new pickers on the in-sketch Offset tool's "Entities" picker and the
  Move/Repeat "Bodies" pickers.
...
- **Elements pane view modes (#34/#252):** two icon-toggle buttons next to the pane heading
  ...
  **taller** rather than overflowing sideways (`declutter_label_bands` returns each node's x and
  ...
  the user can rearrange without fighting the sim. A **Force-layout toggle (#525)** — a
...
- **Timeline rollback (#524):** a rollback marker lets you view the model as it was just after
  a chosen element, temporarily suppressing everything created after it. Selecting a single
  element shows a **⏮ Roll back to here** button atop the Elements pane; the marker then reads
  as `⏮ Rolled back to <name>` with a **Clear** button to roll forward. `shape_order` is the
  timeline (`hierarchy::shape_order_timeline`); `rolled_back_elements` returns every element
  created **after** the marker. Those elements are hidden in the viewport — a render-only
  `ElementVisibility::with_hidden` union on top of the user's own toggles, so their user
  visibility is untouched — and **faded** in the pane (`RowStyle::Faint`, above health/selection
  styling). Per-session UI state; the marker drops automatically once its element is gone
  (deleted, or a new/opened document). Because the marker is a `shape_order` element, this rolls
  back operations that **add** geometry (extrude, mirror, boolean, pattern, sketch geometry, …);
  rolling back an **in-place** modification that has no element of its own — an edge chamfer/
  fillet (#77/#168) — is a tracked follow-up (edge treatments need their own timeline entry).
...
- **Curve vertices (#474):** a sketch vertex on a line or curve is a valid *point* anchor for a
  ...
  meet at the vertex (via coincidence), the Plane tool's context section lists one **Normal**
  choice per incident curve ("Along <line>"), the first picked by default; the context section
  also shows the picked **Anchor** (face, edge, or vertex) as an element-picker row whose ✕
  clears it to re-pick (`construction::vertex_normal_candidates`).
