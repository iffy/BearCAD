...
jobs under `xvfb` + software Vulkan, restores goldens from the Actions cache (not git), and on
master/nightly runs the full suite and refreshes that cache. PRs use `affected` mode (only
changed deps + missing goldens). Goldens are never committed. This reuses §9.3's determinism
guarantees (fixed view, no animation waits).
