...
- **Scriptable gizmos (#214).** Viewport gizmos — a tool's drag handle for its live value,
  ...
  (#185/#215/#216).
- `bearcad.ui.screenshot([path], [whole_window], [window])` captures the 3D viewport only by
  default (the view bear (the view-cube HUD) is suppressed for that frame); passing
  `whole_window = true` captures the entire window. With no `path`, the image is written to
  `screenshot-bearcad.png`. `window` (#347) selects which editor window to capture — `0` (the
  default) is the oldest/main window, `1`+ are extra windows in creation order. Since eframe can
  only screenshot the root viewport, an extra window is captured by rendering its view into the
  root for that one frame; the main window returns to normal on the next frame.
- `bearcad.ui.new_window()` opens a new independent editor window onto the same document (#347;
  native only — a no-op on the web build). See §11.1.
...
- **Multiple windows (#347):** **File → New Window** (native only) opens additional editor
  windows onto the **same document**. Each window is otherwise **independent** — its own camera,
  active tool, selection, sketch session, open drawing and drawing zoom/pan, pane layout and
  element visibility — so you can view/edit the model from several angles or work a drawing beside
  the 3D view at once. Only the document (and its undo history) is shared, so an edit in any window
  shows live in all of them. Each extra window renders the full workspace via `App::draw_workspace`
  into its own eframe *immediate* viewport, with its per-window state swapped into `App` around the
  render (`App::swap_window`). The scriptable `bearcad.ui.new_window()` opens one.
...
- **Elements pane view modes (#34/#252):** two icon-toggle buttons next to the pane heading
  ...
  **taller** rather than overflowing sideways (`declutter_label_bands` returns each node's x and
  ...
  from an element's **inputs** to it (`graph_dependency_edges`): a boolean/move/slice
  operation to its shadow **input bodies** (#266), and a drawing projection to its source
  body/sketch (#281) — the input edges of the eventual full element graph (#252).
