...
- **McMaster-Carr catalog import (#1022):** **File → Import → McMaster-Carr…** (and the Import
  toolbar button's popup) opens **their own site in a window** — a real webview, native child of
  the app's window (`wry`: WKWebView on macOS, WebView2 on Windows, WebKitGTK on Linux) — and
  catches the CAD download on its way out, importing it straight into the document. Pick the part
  the way you already would, choose STEP, and the body lands in the model instead of in Downloads.
  A part number typed in the window's box opens that product page directly; **Catalog** goes back
  to their search.
  **Why the site and not their API:** McMaster's Product Information API is gated behind a signed
  agreement and a client certificate they issue per account, so it would work for almost nobody;
  scraping is against their terms and bot-blocked. Showing the site and catching what the user
  themselves downloaded needs no account and asks nothing of McMaster a browser doesn't.
  **How it lands:** `with_download_started_handler` redirects the file into the app's own scratch
  directory (never the user's Downloads, never a name the URL chose — `download_file_name`
  sanitizes it), and the completed handler posts it to a `CatalogInbox` the frame loop drains.
  A caught file is imported by what it is (`CadFormat`: STEP or STL) through the same
  `import_step_bytes`/`import_stl_bytes` a File → Import goes through, so a catalog part is an
  **ordinary body** afterwards — nothing about it is special, and the scratch copy is deleted.
  It names itself by its part number, taken from the filename or, failing that, from anywhere
  along the URL it came from (a CAD link is `…/91290A115/cad`, so the last segment is the wrong
  thing to take).
  **The window stays McMaster's:** a navigation off their host is refused and handed to the
  user's real browser instead (`is_mcmaster_url`, host-matched so a lookalike domain can't pass),
  rather than quietly becoming a general-purpose browser inside the CAD app.
  The native view composites **above** the wgpu canvas, so the egui window is the frame and the
  chrome and the webview is positioned into its reserved body rect each frame; it therefore also
  floats over anything drawn later, which is why it is the last window drawn.
  **Platforms (`MCMASTER_SUPPORTED`):** macOS and Windows. wry's Linux path needs `gtk::init`
  and a GTK main loop pumped alongside the event loop — which this app doesn't run, and which
  wry *panics* over rather than degrading — and works only on X11; so on Linux the menu item
  and palette entry are absent and the action reports that it needs macOS or Windows rather
  than opening. The web build has no need for it: the browser is already the browser.
  Scriptable: `bearcad.ui.mcmaster("show"|"hide"|"toggle", part?)`.
