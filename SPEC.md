...
- **McMaster-Carr catalog import (#1022):** **File → Import → McMaster-Carr…** (and the Import
  ...
  **The window stays McMaster's:** a navigation off their host is refused and handed to the
  ...
  floats over anything drawn later, which is why it is the last window drawn. Native only — on
  the web build the browser is already the browser. Scriptable: `bearcad.ui.mcmaster("show"|
  "hide"|"toggle", part?)`.
