...
- **Gizmo direction cones:** every gizmo grab handle (plane/extrude/treatment offset
  handles and the axis-plane angle handle) shows small solid cones instead of line-drawn
  arrowheads — one cone per direction the handle can be dragged (both ways along the
  offset normal; both tangent directions on the angle circle), pointing away from the
  handle and offset slightly from its disc. Cones are sized in screen pixels (constant
  on-screen size, like the disc handles) and lightly Lambert-shaded to read as solids;
  the non-GPU 2D painter fallback draws the equivalent filled-triangle silhouettes.
