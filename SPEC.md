...
- **Dimensions:** a newly added projection starts with **no dimensions shown** (#331). The
  ...
  *Hide all* clears them. A detected circle's **diameter dimension is toggleable too** (#342),
  ...
  too short for both arrowheads, they sit **outside the ticks, pointing in**,
  each with a **little reverse arrow out the back**, and the dimension line
  continues through those outside arrows (modeling dimensions do the same). Dimension lines, their extension lines, and diameter lines
...
- **View bear (view-cube HUD) settings popup (#33):** where the projection (orthographic/perspective)
  ...
  - **In-plane dimension-label text (#454):** committed dimension labels lie **in the
    ...
    pointing in**, with a little reverse arrow out the back (same convention as
    drawing dimensions). Glyphs are laid out
