...
- **Every tool input is an element picker (#534):** anything the user can click to feed a
  ...
  registered in `tool_pickers` whatever *where* it draws (§11.4a). The single remaining
  exception is the Construction Plane tool's **Anchor**, which can't hold elements until the
  tool remembers what was picked rather than only the plane it derived. Model new pickers on the
  in-sketch Offset tool's "Entities" picker and the Move/Repeat "Bodies" pickers.
