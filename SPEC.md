...
- **Extrude** — blind, symmetric, to-object, with optional draft angle.
  ...
  - **Boolean-region face picking (#16/#62)**: when exactly two coplanar sketch shapes overlap
    ...
    a subtraction), which is slated for deletion with the lean build mode (todoer #471). Both
...
- **Default feature + CI/release wiring** (#89): `occt` is **on by default**, so the normal
  `cargo build`/`cargo run` ships the kernel (the default `cargo build` therefore needs a built
  OCCT — `scripts/build-occt.sh`/`.ps1` fetch a checksum-verified prebuilt keyed to the pinned
  submodule commit and script hash from the rolling `occt-prebuilt` release, falling back to a
  source build; todoer #469). The lean no-kernel fallback builds with `--no-default-features`.
  ...
  so both paths stay green. **All release binaries — macOS, Linux, and Windows — ship with the
  kernel** (the default build); the static OCCT/MSVC build is exercised by the `windows-occt`
  CI job on every push (#96, todoer #468).
- **Migration status**: extrusions (prism/loft), multi-body union, solid booleans (incl.
  ...
  yet cover (multi-face profiles, imported meshes). With the kernel now the shipping default on
  all platforms, the lean build mode is slated for removal (todoer #470/#471); the hand-rolled
  meshers stay on as the per-frame preview path.
