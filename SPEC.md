...
- **View bear (view-cube HUD) settings popup (#33):** where the projection (orthographic/perspective)
  ...
  - **Rigid dimension-label text (#454):** committed dimension labels in the 3D viewport
    are laid out **rigidly in screen space** — rotated to follow the projected dimension
    line and kept upright, but never sheared, foreshortened, or scaled by perspective
    (`dimensions::planar_label_corners_rigid_screen`). The GPU mesh unprojects that layout
    onto the **camera-facing plane through the label center** (not the sketch plane), so
    the text reprojects pixel-exact and stays legible at any zoom or glancing angle while
    still depth-testing at the label's scene depth.
