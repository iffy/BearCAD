//! The BearCAD Lua API, as a single document the chat model is given (#1623).
//!
//! The chat used to send only the document as Lua. The model then invented calls that
//! do not exist (`bearcad.box{ size = … }`). This catalog is the real API: signatures for
//! the modeling verbs, then every registered function so a name that is not here is not
//! a function.
...
/// Compact on purpose — this is prepended to every chat request.
