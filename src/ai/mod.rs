...
mod setup_tests {
    ...
    fn connecting_stores_a_key_the_user_never_saw_and_asks_what_models_there_are() {
        ...
        // The issued key is a credential like any other: it never reaches a log line, a
        // recorded script, or a debug dump.
        assert!(!format!("{ai:?}").contains(ISSUED));
        assert!(!status.contains(ISSUED));
        let line = crate::script::Instruction::AddAiBackend { backend: backend.clone() }
            .as_lua_in(None);
        assert!(!line.contains(ISSUED), "recorded line leaked the key: {line}");
        drop(ai);
        let _ = provider.join();
    }
}
