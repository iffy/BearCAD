...
pub struct CreatingMove {
    ...
    /// Snap Rotate's source/target face+edge and the chosen orientation (#653).
    pub rotate_source: crate::model::MoveAlignRef,
    pub rotate_target: crate::model::MoveAlignRef,
...
impl AppState {
    ...
    fn resolve_sweep_mode(
        ...
        Ok(match choice {
            ...
            RevolveBodyChoice::AddTouching => {
                if !bodies.is_empty() {
                    return Ok(crate::model::SweepMode::AddTo(bodies.to_vec()));
                }
                ...
                if touching.is_empty() {
                    ...
                    crate::model::SweepMode::AddTo(touching)
                }
            }
            RevolveBodyChoice::Cut => {
                if bodies.is_empty() {
                ...
                crate::model::SweepMode::Cut(bodies.to_vec())
            }
        })
    }

    /// Shared sweep commit: validates the solid builds, stores the
    /// [`crate::model::Sweep`], and (in NewBody mode) its body — one
    /// `ShapeKind::Sweep` undo marker covers both.
    fn create_sweep(
        &mut self,
        sketch: SketchId,
        faces: Vec<ExtrudeFace>,
        ...
        let result = match action {
            ...
            Action::SetTool(tool) => {
                ...
                self.status = match tool {
                    ...
                    Tool::Sweep => {
                        "Sweep tool — click coplanar faces, then a path line".to_string()
                    }
...
label_hidden: false,
            ...
            Action::CreateMoveOperation { translate_mode, source_point, target_point, rotate_mode, rotation_point, extra_rotations, rotate_source, rotate_target, rotate_orientation, targets, plane_targets, image_targets, tx, ty, tz, axis, angle } => {
            ...
            Action::EditMoveOperation { op, translate_mode, source_point, target_point, rotate_mode, rotation_point, extra_rotations, rotate_source, rotate_target, rotate_orientation, targets, plane_targets, image_targets, tx, ty, tz, axis, angle } => {
                ...
                }
                {
                    ...
                    entry.rotate_source = rotate_source;
                    entry.rotate_target = rotate_target;
                    entry.rotate_orientation = rotate_orientation;
                    entry.plane_targets = plane_targets.clone();
            ...
            Action::CommitSweep => {
                let Some(cf) = self.creating_sweep.take() else {
                    return ActionResult::Err("No sweep in progress".to_string());
                };
                ...
                let mode = match self.resolve_sweep_mode(
                    sketch,
                    &cf.faces,
                    &cf.path,
                    cf.body_choice,
                ...
                let result = match cf.editing {
                    Some(op) => {
                        self.edit_sweep(op, sketch, cf.faces.clone(), cf.path.clone(), mode)
                    }
                    None => {
                        self.create_sweep(sketch, cf.faces.clone(), cf.path.clone(), mode)
                    }
                };
                if matches!(result, ActionResult::Err(_)) {
                    self.creating_sweep = Some(cf);
            ...
            Action::CreateSweep { sketch, faces, path, body, bodies } => {
                let mode =
...
fn coalescible_move_op(doc: &Document, cm: &CreatingMove) -> Option<usize> {
    ...
    // A face/edge alignment (#653) isn't an angle either.
    if cm.rotate_source.is_complete() && cm.rotate_target.is_complete() {
        return None;
    }
    doc.move_ops.iter().position(|op| {
        // Extra Free-Rotate slots (#652) can't be folded into one axis+angle either.
        if op.deleted
            || op.has_snap_translation()
            || op.has_extra_rotation()
            || op.has_snap_rotation()
        {
            return false;
        }
