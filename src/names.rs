...
pub fn find_element_by_name(doc: &Document, name: &str) -> Option<SceneElement> {
    ...
    let matches: Vec<SceneElement> = nameable_elements(doc)
        .into_iter()
    ...
    //
    // #1960: and among bodies, the *live* one wins. An operation leaves its input behind as
    // a shadow body carrying the same name; a shadow is hidden from the viewport and left
    // out of export, so it is never what a lookup by name wants. Resolving to one turns
    // every following call into a silent no-op on geometry that is not in the scene.
    let is_live_body = |element: &SceneElement| {
        matches!(element, SceneElement::Body(b) if doc.bodies.get(*b).is_some_and(|body| !body.shadow))
    };
    if let Some(live) = matches.iter().find(|e| is_live_body(e)) {
        return Some(live.clone());
    }
    if let Some(body) = matches.iter().find(|e| matches!(e, SceneElement::Body(_))) {
...
pub fn element_name(doc: &Document, element: SceneElement) -> Option<&str> {
    let name = match element {
        ...
        SceneElement::ShellOp(index) => doc.shell_ops.get(index)?.name.as_deref(),
    ...
    match element {
        ...
        SceneElement::ShellOp(index) => {
            let op = doc
                .shell_ops
                .get_mut(index)
                .ok_or_else(|| format!("shell operation {index:?} not found"))?;
            op.name = stored;
        }
        SceneElement::EdgeTreatmentOp(index) => {
            let op = doc
...
pub fn default_node_label(doc: &Document, node: HierarchyNode) -> String {
    match node {
        ...
        HierarchyNode::ShellOp(i) => format!("Shell {}", i.index()),
...
pub fn scene_element_label(doc: &Document, element: &SceneElement) -> String {
    ...
    match element {
        ...
        SceneElement::ShellOp(i) => format!("Shell {}", i.index()),
