...
2. **Add view:** pick the **Add view** tool (the ＋ in the toolbar), then click a body or
   ...
   direction is shown as a **blue highlight** on the matching face of the bear, so you can tell
   at a glance which way it's looking (an isometric view has no single face, so it isn't
   highlighted).
