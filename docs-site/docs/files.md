...
- **File → Export → STL…** / **3MF…** — for 3D printing (mesh). Right-click a body row in the
  ...
  object and writes its [material](/docs/materials) color (`m:colorgroup`), so multi-color models open in
  Bambu Studio with each color already on its own filament slot. **Shadow bodies** (operation
