...
- **File → Export → STL…** — for 3D printing. Right-click a body row in the Elements pane to
  ...
  nested components) as one file. **Shadow bodies** (operation inputs, or any body marked
  **Make shadow body** in the Elements pane) are left out of whole-document and component export.
