-- Documentation screenshot: the Fillet tool's Context pane with help mode on (#672).
--
-- Fillet asks for different things in a sketch than on a solid, so both are captured:
-- in a sketch it collects corners, and on a body it collects edges. Neither carries the
-- radius — that is typed in the viewport once something is picked.
--
-- Output dir: $BEARCAD_SCREENSHOT_OUT (set by scripts/gen-doc-screenshots.sh),
-- falling back to ".". Two PNGs.

local out = (os.getenv("BEARCAD_SCREENSHOT_OUT") or ".") .. "/pane-fillet"
