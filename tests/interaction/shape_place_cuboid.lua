...
local size = {
  stats.bbox.max.x - stats.bbox.min.x,
  stats.bbox.max.y - stats.bbox.min.y,
  stats.bbox.max.z - stats.bbox.min.z,
}
...
local diameter = stats.bbox.max.x - stats.bbox.min.x
assert(math.abs(diameter - 30) < 1.5, "30 mm across, got " .. diameter)
assert(math.abs((stats.bbox.max.z - stats.bbox.min.z) - 10) < 0.1,
  "the typed 10 mm height, got " .. (stats.bbox.max.z - stats.bbox.min.z))
...
assert(math.abs(stats.bbox.min.z - 12) < 0.2,
  "the sphere rests on the cuboid's 12 mm top face, got " .. stats.bbox.min.z)
assert(math.abs((stats.bbox.max.z - stats.bbox.min.z) - 12) < 0.2,
  "and is 2 x 6 mm across, got " .. (stats.bbox.max.z - stats.bbox.min.z))
