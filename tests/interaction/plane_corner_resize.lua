...
local before = extent()
assert(math.abs(before.u_max - 105.0) < 1e-2 and math.abs(before.v_max - 105.0) < 1e-2,
  string.format("the XY datum plane starts 100x100 with a 5mm gap, got %.1f x %.1f",
                before.u_max, before.v_max))

-- Drag the far (u_max, v_max) grip: wider in u, shorter in v.
bearcad.ui.drag_ground(105, 105, 140, 60)
bearcad.ui.wait(20)
local after = extent()
...
assert(math.abs(after.u_min - 5.0) < 1e-2 and math.abs(after.v_min - 5.0) < 1e-2,
...
bearcad.ui.wait(10)
local undone = extent()
assert(math.abs(undone.u_max - 105.0) < 1e-2 and math.abs(undone.v_max - 105.0) < 1e-2,
...
bearcad.select{ kind = "construction_plane", index = 0 }
bearcad.ui.wait(3)
bearcad.ui.drag_ground(5, 5, -30, 20)
bearcad.ui.wait(20)
local low = extent()
