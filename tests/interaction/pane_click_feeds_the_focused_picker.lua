...
-- A construction plane is not a valid target, so it must not land in Targets.
bearcad.select{ kind = "plane", index = 0 }
bearcad.ui.wait(5)
assert(#picker("Targets").items == 1,
  "a plane is not a body target, got " .. #picker("Targets").items)

-- Arm Cutters, then the same plane is exactly what it wants.
bearcad.ui.picker_focus("Cutters")
bearcad.ui.wait(5)
assert(picker("Cutters").focused, "Cutters should now be focused")
bearcad.select{ kind = "plane", index = 0 }
bearcad.ui.wait(5)
