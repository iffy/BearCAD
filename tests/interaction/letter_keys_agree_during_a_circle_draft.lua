...
bearcad.ui.wait(5)
assert(bearcad.debug.tool_row().tool == "circle", "circle draft should be armed")

for _, key in ipairs({ "r", "l", "d", "e", "m", "k", "f", "s" }) do
  bearcad.ui.key(key)
  bearcad.ui.wait(2)
  assert(bearcad.debug.tool_row().tool == "circle",
    "mid-circle, " .. key .. " must not switch tools, got " .. bearcad.debug.tool_row().tool)
end
